...
# def get_df(wc):
#     word_counter = collections.Counter(wc).most_common(20)
...
def get_df(wc):
    #trimmed_count = collections.Counter(wc).most_common(20)
    words = list(wc.keys())
    counts = list(wc.values())

    data = {"Word": words,"Count": counts}
...
# writer = pd.ExcelWriter("user_interests.xlsx", engine='xlsxwriter')
# company_freq.to_excel(writer, index =False)
# writer.save()
